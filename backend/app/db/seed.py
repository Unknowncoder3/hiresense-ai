...
from app.models import Job, Candidate, Application, Interview
    ...
    try:
        ...
        for i, name in enumerate(names, start=1):
            candidates.append(Candidate(
                name=name, email=f"candidate{i}@example.com", location=rng.choice(["Bengaluru","Hyderabad","Kolkata","Pune","Mumbai","Remote"]),
                experience_years=round(rng.uniform(0.5, 5.5), 1), skills=rng.choice(skills), education=rng.choice(["B.Tech Computer Science","B.Tech CSBS","M.Sc Data Science","BCA","MBA Analytics"]),
                source=rng.choice(["LinkedIn","Referral","Company Website","Job Portal","Campus"]), created_at=date(2026, 6, rng.randint(1, 28))))
        db.add_all(candidates); db.flush()
        stages=["Applied","Screening","Shortlisted","Interview","Offer","Hired"]
        applications=[]
        for i, c in enumerate(candidates):
            job = rng.choice(jobs[:4])
            stage = rng.choices(stages, weights=[25,20,18,15,10,12])[0]
        ...
        for app in applications:
            if app.stage in ["Interview", "Offer", "Hired"]:
                db.add(Interview(application_id=app.id, interview_type=rng.choice(["Technical","Hiring Manager","HR"]), scheduled_at=app.applied_at+timedelta(days=rng.randint(5,16)), score=round(rng.uniform(62,98),1), outcome="Passed" if app.stage in ["Offer","Hired"] else "Pending"))
        db.commit()
