...
export function getCandidates(params={}){const q=new URLSearchParams(Object.entries(params).filter(([,v])=>v&&v!=='All'));return request(`/candidates${q.toString()?`?${q}`:''}`)}
export function createCandidate(payload){return request('/candidates',{method:'POST',body:JSON.stringify(payload)})}
export function getJobs(params={}){const q=new URLSearchParams(Object.entries(params).filter(([,v])=>v&&v!=='All'));return request(`/jobs${q.toString()?`?${q}`:''}`)}
export function createJob(payload){return request('/jobs',{method:'POST',body:JSON.stringify(payload)})}
export function getApplications(){return request('/applications')}
export function updateApplicationStage(id,stage){return request(`/applications/${id}/stage`,{method:'PATCH',body:JSON.stringify({stage})})}
export function getInterviews(){return request('/interviews')}
export function createInterview(payload){return request('/interviews',{method:'POST',body:JSON.stringify(payload)})}
